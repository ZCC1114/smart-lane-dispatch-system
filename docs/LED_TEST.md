...
- **现场网格布局**：支持按指定屏幕尺寸、列数和行数切分显示区域。
- **总入口引导数据**：测试页可读取 `/api/screen/board` 中的引导数据，按 `车牌-车道` 格式发送。
...
确保电脑与显示屏处于同一网段。现场 LED 当前按以下参数预置：
