  ...
  return (
    <article className="overflow-hidden rounded-sm border border-[var(--border-soft)] bg-[var(--bg-panel-strong)] transition-colors hover:border-blue-300">
      ...
      <div className="bg-slate-50/70 p-6">
        <div className="mb-4 flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">当前排队 / 通行</p>
            <p className="font-mono text-3xl font-bold tracking-wide text-blue-700">{lane.currentPlate ?? `${lane.zone}-${lane.vehicleCount}`}</p>
          </div>
          <div>
            {signal === "OFFLINE" ? (
              ...
              <div className="flex items-center space-x-1.5 rounded-sm border border-slate-200 bg-white px-3 py-1 shadow-sm">
                ...
                <div className={cn("size-2 rounded-full", signal === "YELLOW" ? "bg-amber-400 shadow-[0_0_8px_rgba(234,179,8,0.45)]" : "bg-slate-200")} />
      ...
      <div className="flex gap-2 border-t border-[var(--border-soft)] bg-white px-4 py-3">
        ...
        <Link
          href={canOperate ? "/dispatch" : "/alerts"}
